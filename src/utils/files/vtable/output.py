...
from src.lib.dsv import writer
import gzip
from src.lib.ziputils import ZipIter
import functions
from src.lib.vtoutgtable import vtoutpugtformat
...
def outputData(diter, schema, connection, *args, **formatArgs):
    ...
    try:
        ...
        if formatArgs['mode'] == 'json':
            ...
            for row in diter:
                csvprinter.writerow(
                    [x.replace('\t', '    ') if type(x) is str or type(x) is str else x for x in row])
            ...
            def createdb(where, tname, schema, page_size=16384):
                ...
                create_schema += '`' + str(schema[0][0]) + '`' + (
                    ' ' + str(schema[0][1]) if schema[0][1] != None else '')
...
# boolargs = src.lib.inoutparsing.boolargs + ['append', 'header', 'compression']


# def Source():
#     global boolargs, nonstringargs
#     return SourceNtoOne(outputData, boolargs, src.lib.inoutparsing.nonstringargs, src.lib.inoutparsing.needsescape,
