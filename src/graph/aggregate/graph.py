...
class graphtodot:
    ...
    def final(self):
        ...
        if type(self.graphname) in (int, float):
            self.graphname = u'g' + str(self.graphname)
        else:
...
class graphtotgf:
    ...
    def final(self):
        ...
        for n, v in self.nodes.items():
            for e in v[0]:
                tgf += clearname(n) + ' ' + clearname(e[0]) + ' ' + (clearname(e[1]) if e[1] is not None else '') + '\n'
