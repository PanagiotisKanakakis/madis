import re
from src.lib.unicodeops import unistr
...
def unquote(p):
    if p.startswith(str("'").encode('utf-8')) and p.endswith(str("'").encode('utf-8')):
        return p[1:-1].replace("''", "'")
    elif p.startswith(str('"').encode('utf-8')) and p.endswith(str('"').encode('utf-8')):
        return p[1:-1].replace('""', '"')
    return p
